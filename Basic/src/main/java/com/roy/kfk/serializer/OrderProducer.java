...
public class OrderProducer {
    ...
    public static void main(String[] args) throws ExecutionException, InterruptedException {
        ...
        // 配置value的序列化类
        props.put(ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG,"com.roy.kfk.serializer.OrderSerializer");

        Producer<String,Order> producer = new KafkaProducer<>(props);
//        CountDownLatch latch = new CountDownLatch(5);
        for(int i = 0; i < 5; i++) {
            Order order = new Order((long)i, "order"+i, i%2);
            //Part2:构建消息
            ProducerRecord<String, Order> record = new ProducerRecord<>(TOPIC, Integer.toString(i), order);
            //Part3:发送消息
            //单向发送：不关心服务端的应答。
